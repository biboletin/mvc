...
class Stream implements StreamInterface
{
    ...
    public function __construct($body = null)
    {
        if (!is_string($body) && !is_resource($body) && $body !== null) {
            throw new Exception('Argument 1 must be string, resource or null');
        }
    ...
    {
        if ($this->stream === null) {
            throw new Exception('Unable to get current position');
        }
    ...
    public function seek($offset, $whence = SEEK_SET): void
    {
        ...
        if (fseek($this->stream, $offset, $whence) === -1) {
            throw new Exception('Unable to seek stream position ' . $offset);
        }
    }
    ...
    public function read($length): string
    {
        ...
        if ($result === false) {
            throw new Exception('Unable to read the stream');
        }
